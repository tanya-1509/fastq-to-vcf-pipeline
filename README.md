...
This repository is a guide to create a fastq to vcf pipeline using GATK best practices for processing raw high throughput sequencing data from NGS platforms like Illumina to generate a variant call file which can be used for further downstream analysis.
...
## WSL2
The FASTQ to VCF pipeline execution was done on `WSL2`.

Windows Subsystem for Linux (WSL) is a feature of Windows that allows you to run a Linux environment on your Windows machine, without the need for a separate virtual machine or dual booting. WSL is designed to provide a seamless and productive experience for developers who want to use both Windows and Linux at the same time.

Follow [WSL installation](https://learn.microsoft.com/en-us/windows/wsl/install) to get WSL on your Windows machine.

## Bioinformatics Tools
### 1. BWA (Burrows Wheeler Aligner)
   * **Usage:** This tool is used for aligning sequencing reads (from FASTQ files) to a reference genome (like the human genome in fasta format). It's particularly efficient for mapping short and long DNA reads generated by platforms like Illumina. Check [BWA](https://bio-bwa.sourceforge.net/) for more details.
   * **Installation:**
        1. PC requirement:
             * Linux/MacOS Environment
             * GNU make
             * C complier (eg. gcc or clang)
        2. Download and extract bwa:
             * Visit [bwa github page - Releases](https://github.com/lh3/bwa/releases) and download the source code for the latest bwa version.
               ```
               wget https://github.com/lh3/bwa/archive/refs/tags/v0.7.18.zip
               ```
             * Extract the contents of the zip file to your desired location:
               ```
               unzip v0.7.18.zip
               ```
        3. Compile the files inside the bwa directory using `make`:
           ```
           cd /path/to/bwa-0.7.18/
           make
           ```
        4. Add `bwa` to your system's PATH:\
           Edit your .bashrc file adding the location of the directory containing `bwa`. This allows you to run bwa commands from anywhere in your terminal.
           ```
           export PATH="$PATH:/path/to/your/bwa/directory/bwa-0.7.18"
           ```
           ***Please follow this tutorial [BWA installation](https://youtu.be/haXr1UkaRZU?si=-stX6eIQFI2u2hO5) for a detailed explanation.***
* **Input and Output files:**
    * Input: FASTQ files (reads), FASTA (reference)
    * Output: SAM file

### 2. Samtools
  * **Usage:**
    In this pipeline, samtools is used to convert a SAM (Sequence Alignment Map) file to a sorted BAM (Binary Alignment Map) file. Its also used to index the BAM files. Check [samtools](https://www.htslib.org/) for more details.
  * **Installation:**
      1. PC requirement:
           * Linux/MacOS Environment
           * GNU make
           * C complier (eg. gcc or clang)
           * zlib <http://zlib.net>
           * curses or GNU ncurses (optional, for the 'tview' command) <http://www.gnu.org/software/ncurses/>
      2. Download and extract samtools:
           * Visit [samtools releases](https://www.htslib.org/download/) and download the source code for the latest samtools version. It can also be downloaded from [samtools github page - Releases](https://github.com/samtools/samtools/releases)
             ```
             wget https://github.com/samtools/samtools/releases/download/1.21/samtools-1.21.tar.bz2
             ```
           * Extract the contents of the tar file to your desired location:
             ```
             tar -xvf samtools-1.21.tar.bz2
             ```
      3. Build and install `samtools`:
         ```
         cd /path/to/samtools-1.21
         ./configure --prefix=/where/to/install
         make
         make install
         ```
      4. Add `samtools` to your system's PATH:\
           Edit your .bashrc file adding the location of the directory containing `samtools`. This allows you to run samtools commands from anywhere in your terminal.
           ```
           export PATH="$PATH:/path/to/your/samtools/directory/samtools-1.21"
           ```
           ***Please follow this tutorial [Samtools installation](https://youtu.be/ULQU8X3abxY?si=-AczRRDFN10jY0I8) for detailed explanation.***
...
### 3. GATK4
  * **Usage:** Geneome Analysis Toolkit (GATK) is a collection of command-line tools for analyzing high-throughput sequencing data with a primary focus on variant discovery. Check [GATK](https://gatk.broadinstitute.org/hc/en-us) for more details.
   * **Installation:**
       1. To run GATK:
       ...
       2. Download and extract GATK4:
            * Visit the [Broad Institute's GATK github page](https://github.com/broadinstitute/gatk/releases) and download the latest GATK4 version:
              ```
            ...
            * Extract the contents of the zip file to your desired location:
              ...
              ```
        3. Add GATK to your system's PATH:\
           ...
           ```
           export PATH="$PATH:/path/to/your/gatk/directory/gatk-4.6.1.0"
           ```
           ***Please follow [GATK installation](https://youtu.be/haXr1UkaRZU?si=-stX6eIQFI2u2hO5) for detailed explanation.***
...
1. `samtools` here is used to generate a sorted BAM file from a SAM file.
2. **Command:**
   ```
...
3. **Parameter breakdown:**
     ...
     * `rm sample_name.sam` - Removes the SAM file as it not need for upcoming steps.
...
## Pipeline Execution
For smooth execution of the pipeline, its better to run the above mentioned commands individually first and debug if any errors arise. Post that compile all the commands in a shell script and run the script on the terminal:
```
